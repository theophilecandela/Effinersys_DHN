...
def simulation( RES, Ts2_h, Tsupply_h = Tint):
    ...
    for j in range(24):
        ...
        for m in range(60):
            ...
            T_return_nodes.append(RES.Tr_nodes)
    ...
    plt.figure()
    plt.plot(t, T_return, label = 'Temperature de retour')
    for i in range(len(T_return_nodes[0])):
        plt.plot(t, [a[i] for a in T_return_nodes], label= f'T_return {i}')
    plt.legend()
    
    plt.figure()
    plt.plot(t, m_dot, label = 'debit réseau')
    plt.legend()
    
    plt.figure()
    plt.plot(t, T_in, label = 'T_in réseau')
    for i in range(len(T_supplySS[0])):
        plt.plot(t, [a[i] for a in T_supplySS], label = 'T_supply SS réseau')
    plt.legend()
    ...
    for i in range(len(T_supply_secondary[0])):
        ...
        plt.plot(t, [a[i] for a in T_supplySS], label = 'T_supply SS réseau')
        plt.plot(t, [a[i] for a in T_return_nodes], label = 'T_return SS réseau')
        plt.legend()   
        # plt.figure()
...
SS1 = [HEX(70, 45, 44, 32, 1.6, 2.1), Pipe(lam_i, lam_p, lam_s, R_int, R_p, R_i, z, 1000), Pipe(lam_i, lam_p, lam_s, 120e-3, 145e-3, 195e-3, z, 50)]
SS2 = [HEX(70, 45, 44, 25, 1.8, 2.1), Pipe(lam_i, lam_p, lam_s, R_int, R_p, R_i, z, 500), Pipe(lam_i, lam_p, lam_s, 120e-3, 145e-3, 195e-3, z, 50)]
SS3 = [HEX(70, 45, 44, 25, 1.8, 2.1), Pipe(lam_i, lam_p, lam_s, R_int, R_p, R_i, z, 2500), Pipe(lam_i, lam_p, lam_s, 120e-3, 145e-3, 195e-3, z, 50)]
