    ...
    def simulation(self, P_boiler_instruction):
        ...
        for j in range(24):
            ...
            for m in range(60):
                ...
                if maxT > 85:
                    self.cost_constraintT += 0 #3*(np.exp(0.2*(maxT-95)) - np.exp(-2)) #+ np.exp(100 * (maxT - 95))
                if maxT > 115:
                    return 10* (maxT - 115)
...
algorithm_param={'max_num_iteration': 60, 'population_size': 50, 'mutation_probability': 0.1, 'elit_ratio': 0.2, 'crossover_probability': 0.65, 'parents_portion': 0.3, 'crossover_type': 'uniform', 'max_iteration_without_improv': None}

def optimaff(dim = 2, MOD= A1):
    varbound=np.array([[0,90000]]*dim)
